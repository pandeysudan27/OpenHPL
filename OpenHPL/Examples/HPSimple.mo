within OpenHPL.Examples;
model HPSimple "Model of waterway of the HP system with simplified models for conduits, turbine, etc."
  extends Modelica.Icons.Example;
  OpenHPL.Waterway.Reservoir reservoir(H_r=48) annotation (Placement(visible=true, transformation(
        origin={-90,30},
        extent={{-10,-10},{10,10}},
        rotation=0)));
  Modelica.Blocks.Sources.Ramp control(
    duration=0,
    height=-0.9,
    offset=1,
    startTime=1000)                                                                                       annotation (
    Placement(visible = true, transformation(origin={-10,70},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  OpenHPL.Waterway.Pipe intake(H=23) annotation (Placement(visible=true, transformation(extent={{-70,20},{-50,40}}, rotation=0)));
  OpenHPL.Waterway.Pipe discharge(H=0.5, L=600) annotation (Placement(visible=true, transformation(extent={{50,-10},{70,10}}, rotation=0)));
  OpenHPL.Waterway.Reservoir tail(H_r=5, Input_level=false) annotation (Placement(visible=true, transformation(
        origin={90,0},
        extent={{-10,10},{10,-10}},
        rotation=180)));
  OpenHPL.Waterway.SurgeTank surgeTank(h_0=69.9) annotation (Placement(visible=true, transformation(
        origin={-30,30},
        extent={{-10,-10},{10,10}},
        rotation=0)));
  ElectroMech.Turbines.Turbine turbine(
    ValveCapacity=false,               C_v=3.7, ConstEfficiency=false) annotation (Placement(visible=true, transformation(
        origin={30,10},
        extent={{-10,-10},{10,10}},
        rotation=0)));
  inner OpenHPL.Data data annotation (Placement(visible=true, transformation(
        origin={-90,90},
        extent={{-10,-10},{10,10}},
        rotation=0)));
  Waterway.PenstockKP penstockKP
    annotation (Placement(transformation(extent={{-8,14},{12,34}})));
  Waterway.Reservoir         reservoir1(H_r=48)
                                               annotation (Placement(visible=true, transformation(
        origin={-90,-46},
        extent={{-10,-10},{10,10}},
        rotation=0)));
  Modelica.Blocks.Sources.Ramp control1(
    duration=0,
    height=-0.99,
    offset=1,
    startTime=1500)                                                                                       annotation (
    Placement(visible = true, transformation(origin={-10,-6},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Waterway.Pipe         intake1(H=23)
                                     annotation (Placement(visible=true, transformation(extent={{-70,-56},
            {-50,-36}},                                                                                             rotation=0)));
  Waterway.Pipe         discharge1(H=0.5, L=600)
                                                annotation (Placement(visible=true, transformation(extent={{50,-86},
            {70,-66}},                                                                                                        rotation=0)));
  Waterway.Reservoir         tail1(H_r=5, Input_level=false)
                                                            annotation (Placement(visible=true, transformation(
        origin={90,-76},
        extent={{-10,10},{10,-10}},
        rotation=180)));
  Waterway.SurgeTank         surgeTank1(h_0=69.9)
                                                 annotation (Placement(visible=true, transformation(
        origin={-30,-46},
        extent={{-10,-10},{10,10}},
        rotation=0)));
  ElectroMech.Turbines.Turbine turbine1(
    ValveCapacity=false,
    C_v=3.7,
    Vdot_n=20,
    ConstEfficiency=false)                                             annotation (Placement(visible=true, transformation(
        origin={30,-66},
        extent={{-10,-10},{10,10}},
        rotation=0)));
  Waterway.Penstock penstock
    annotation (Placement(transformation(extent={{-8,-70},{12,-50}})));
equation
  connect(turbine.o, discharge.i) annotation (
    Line(points={{40,10},{44,10},{44,0},{50,0}},            color = {28, 108, 200}));
  connect(control.y, turbine.u_t) annotation (
    Line(points={{1,70},{30,70},{30,22}},         color = {0, 0, 127}));
  connect(reservoir.o, intake.i) annotation (
    Line(points={{-80,30},{-70,30}},                                              color = {28, 108, 200}));
  connect(intake.o, surgeTank.i) annotation (
    Line(points={{-50,30},{-40,30}},                                              color = {28, 108, 200}));
  connect(discharge.o, tail.o) annotation (Line(points={{70,0},{80,0}}, color={28,108,200}));
  connect(surgeTank.o, penstockKP.i) annotation (Line(points={{-20,30},{-14,30},
          {-14,24},{-8,24}}, color={28,108,200}));
  connect(turbine.i, penstockKP.o) annotation (Line(points={{20,10},{16,10},{16,
          24},{12,24}}, color={28,108,200}));
  connect(turbine1.o, discharge1.i) annotation (Line(points={{40,-66},{44,-66},
          {44,-76},{50,-76}}, color={28,108,200}));
  connect(control1.y, turbine1.u_t)
    annotation (Line(points={{1,-6},{30,-6},{30,-54}}, color={0,0,127}));
  connect(reservoir1.o, intake1.i)
    annotation (Line(points={{-80,-46},{-70,-46}}, color={28,108,200}));
  connect(intake1.o, surgeTank1.i)
    annotation (Line(points={{-50,-46},{-40,-46}}, color={28,108,200}));
  connect(discharge1.o, tail1.o)
    annotation (Line(points={{70,-76},{80,-76}}, color={28,108,200}));
  connect(surgeTank1.o, penstock.i) annotation (Line(points={{-20,-46},{-14,-46},
          {-14,-60},{-8,-60}}, color={28,108,200}));
  connect(turbine1.i, penstock.o) annotation (Line(points={{20,-66},{16,-66},{
          16,-60},{12,-60}}, color={28,108,200}));
  annotation (
    experiment(StopTime = 2000, StartTime = 0, Tolerance = 0.0001, Interval = 0.4));
end HPSimple;
